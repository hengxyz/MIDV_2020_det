...
# -*- coding: utf-8 -*-
"""
Created on Thu May 30 16:33:13 2019
...
cbdar_path = '/data/zming/datasets/CBDAR/old_2'


#evaluate_data = 'evaluate_pcn_MIDV_v2.txt'
evaluate_data = 'evaluate_mtcnn_MIDV_v2.txt'
#evaluate_data = 'MIDV-500-outList_cascade.txt'
        ...
        for line in lines:
            ...
            for i in range(gt_num):
                ...
                cv2.rectangle(img, (int(gt_bboxes[i][0]), int(gt_bboxes[i][1])),
                              (abs(int((gt_bboxes[i][2]-gt_bboxes[i][0]))), abs(int((gt_bboxes[i][3]-gt_bboxes[i][1])))), (255, 0, 0), 3)
            ...
            if pre_num == 0:
                ...
                for i in range(pre_num):
                    ...
                    ## draw the predicted boxes for MTCNN & Cascade-CNN
                    shfit_pixel = 5
                    cv2.rectangle(img, (int(pre_bboxes[i][0]+shfit_pixel), int(pre_bboxes[i][1]++shfit_pixel)),
                                  (abs(int(pre_bboxes[i][2])+shfit_pixel), abs(int(pre_bboxes[i][3])+shfit_pixel)), (0, 255, 0), 3, 8)
                    
                    ## draw the predicted boxes for PCN
                   # shfit_pixel = 5
                    #cv2.rectangle(img, (int(pre_bboxes[i][0]+shfit_pixel), int(pre_bboxes[i][1]+shfit_pixel)),
                                  #(abs(int(pre_bboxes[i][2]-pre_bboxes[i][0])+shfit_pixel), abs(int(pre_bboxes[i][3]-pre_bboxes[i][1])+shfit_pixel)), (0, 255, 0), 3, 8)
