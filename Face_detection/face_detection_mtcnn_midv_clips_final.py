...
for i, sub in enumerate(subjects):
    det_sub = os.path.join(path_det, "images", sub)
    if not os.path.isdir(det_sub):
        os.mkdir(det_sub)
    sub_folders = os.listdir(os.path.join(path_vids,"images", sub))
    sub_folders.sort()
    for sub_folder in sub_folders:
        det_sub_folder = os.path.join(path_det, "images", sub, sub_folder)
        if not os.path.isdir(det_sub_folder):
            os.mkdir(det_sub_folder)
        images = glob.glob(os.path.join(path_vids, "images", sub, sub_folder, '*.jpg'))
        images.sort()
        bbox_np = np.zeros(4)
